...
local list_acc = {}
local OUTPUT_FILE = "Delta_Saldo_Giro"
local f_lines1 = nil
local f_lines2 = nil
...
for line in f_lines1(ReportFileName1) do
	...
	if no == 1 then
		...
		if report1_type == nil then
			iup.Message("Error","Report [Awal] yang dipilih bukan Report \"DI321PN\" atau \"DI321\" dalam format CSV.\nSilahkan download ulang dari BRISIM atau \npilih kembali report yang sesuai.")
			return -1
		end
	else
		-- process data
		...
		if f[1] ~= "" then
			if (is_foreign == 1 and f[3] ~= "IDR ") or (is_foreign == 0 and f[3] == "IDR ") then
				posisi_report1 = f[1]
				acc_no = f[6]
				if report1_type == "DWH-20-DI321" then
					acc_cif = '-'
				elseif report1_type == "EDW-176-DI321" or report1_type == "EDW-176-DI321v2" then
					acc_cif = f[5]..' - '..f[7]
				else
					acc_cif = f[5]
				end
				...
				if is_foreign == 1 then acc_name = acc_name..' <b>'..f[3]..'</b>' end
				...
				if report1_type == "DWH-7-DI321" then
					acc_officer = f[17]..' - '..f[18]
				elseif report1_type == "DI321MULTIPN" then
					acc_officer = Rekap_Officer(f[17],f[18],f[19],f[20],f[21],f[22],f[23],f[24])
				elseif report1_type == "EDW-176-DI321" then
					acc_officer = Rekap_Officer(f[19],f[20],f[21],f[22],f[23],f[24],f[25],f[26])
				elseif report1_type == "EDW-176-DI321v2" then
					acc_officer = Rekap_Officer(f[18],f[19],f[20],f[21],f[22],f[23],f[24],f[25])
				else	--report1_type == "DWH-20-DI321"
					acc_officer = ''
				end
...
for line in f_lines2(ReportFileName2) do
	...
	if no == 1 then
		...
		if report2_type == nil then
			iup.Message("Error","Report [Akhir] yang dipilih bukan Report \"DI321PN\" atau \"DI321\" dalam format CSV.\nSilahkan download ulang dari BRISIM atau \npilih kembali report yang sesuai.")
			return -1
		end
	else
		f = csv.parse(line, sep)
		...
		if f[1] ~= "" then
			if (is_foreign == 1 and f[3] ~= "IDR ") or (is_foreign == 0 and f[3] == "IDR ") then
				posisi_report2 = f[1]
				acc_no = f[6]
				if report2_type == "DWH-20-DI321" then
					acc_cif = '-'
				elseif report2_type == "EDW-176-DI321" or report2_type == "EDW-176-DI321v2" then
					acc_cif = f[5]..' - '..f[7]
				else
					acc_cif = f[5]
				end
				...
				if report2_type == "DWH-7-DI321" then
					acc_officer = f[17]..'-'..f[18]
				elseif report2_type == "DI321MULTIPN" then
					acc_officer = Rekap_Officer(f[17],f[18],f[19],f[20],f[21],f[22],f[23],f[24])
				elseif report2_type == "EDW-176-DI321" then
					acc_officer = Rekap_Officer(f[19],f[20],f[21],f[22],f[23],f[24],f[25],f[26])
				elseif report2_type == "EDW-176-DI321v2" then
					acc_officer = Rekap_Officer(f[18],f[19],f[20],f[21],f[22],f[23],f[24],f[25])
				else
					acc_officer = ''
				end
...
table td.plus {
 ...
 <body>
 <h2>&nbsp;&nbsp;&nbsp;REKENING GIRO YANG SALDONYA TURUN ]=]..string.format("(Periode %s - %s) %s",posisi_report1, posisi_report2, report2_type)..[=[</h2>
 <table cellspacing='0'>
	<thead>
		<tr>
			<th>No Rekening</th>
...
for k, v in pairs(sorted_list_acc) do	
	if ii < limit_res then
		if tonumber(v[6]) ~= 0 then
			fo2:write("<tr><td>"..format_account(v[1]).."</td><td>"..v[2].."</td><td>"..v[3].."</td><td align='right'>"..format_number(v[4]).."</td><td align='right'>"..format_number(v[5]).."</td><td class='minus' align='right'>"..format_number(v[6]).."</td><td align='left'>"..v[7].."</td></tr>\n")
		end
	elseif ii == limit_res then
		fo2:write([=[
	</tbody>
 </table>
 <hr><br><h2>&nbsp;&nbsp;&nbsp;REKENING GIRO YANG SALDONYA NAIK ]=]..string.format("(Periode %s - %s) %s",posisi_report1, posisi_report2, report2_type)..[=[</h2>
 <table cellspacing='0'>
	<thead>
		<tr>
			<th>No Rekening</th>
	...
	<tbody>
		...
		if (#sorted_list_acc - ii) <= limit_res then
			if tonumber(v[6]) ~= 0 then
				fo2:write("<tr><td>"..format_account(v[1]).."</td><td>"..v[2].."</td><td>"..v[3].."</td><td align='right'>"..format_number(v[4]).."</td><td align='right'>"..format_number(v[5]).."</td><td class='plus' align='right'>"..format_number(v[6]).."</td><td align='left'>"..v[7].."</td></tr>\n")
			end
		end
	end
