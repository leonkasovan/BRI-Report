-- Display Performance each funding officer based on DI321PN EDW Current Account Balance (CSV Format from DWH BRISIM)
...
local OUTPUT_FILE = "Akuisisi_Bulanan_Giro.htm"
local OUTPUT_NO_PN_FILE = "Giro_tanpa_PN.csv"
local output_sep = "|"

local res, tmp, pengelola, ReportFileName1 = iup.GetParam("Pick Reports DI321 in CSV Format", nil, [=[
Sumber Data : %m\n
%t\n
			...
			if string.sub(acc_dateopen,7) == this_year then
				...
				if acc_RM ~= "" then
					...
					if list_RM[acc_RM] == nil then
						...
						lRM = list_RM[acc_RM]
						lRM[mm] = 1
						lRM[mm+12] = string.format("%s - %s [%s]",f[6] ,f[9], f[7])
						lRM["total_account"] = 1
						...
						-- if #(lRM[mm+12]) < 1024 then
							lRM[mm+12] = lRM[mm+12]..'&#13;'..string.format("%s - %s [%s]",f[6] ,f[9], f[7])
						-- end
					...
					if f[27]=="0" then	-- rekening tidak ber PN
						...
						if list_RM[acc_RM] == nil then
							...
							, lRM["total_account"], output_sep, f[6], output_sep, f[9], output_sep, f[7], output_sep, format_number(acc_balance))
							, OUTPUT_NO_PN_FILE)
						else
							lRM = list_RM[acc_RM]
							lRM[mm] = lRM[mm] + 1
							...
							lRM["total_account"] = lRM["total_account"] + 1
							lRM["total_balance"] = lRM["total_balance"] + acc_balance
							save_file(string.format("%03d%s%s%s%s%s%s%s%s\n", lRM["total_account"], output_sep, f[6], output_sep, f[9], output_sep, f[7], output_sep, format_number(acc_balance)), OUTPUT_NO_PN_FILE)
						end
					end
				end
			end
...
this_month = tonumber(string.sub(posisi_report1,4,5))
...
<title>BRI Reporting Tool: Akuisisi Giro DI321</title>
<style type="text/css"> 
...
window.onload = function () {makeAllSortable();};
 ...
 <body>
 <h2>&nbsp;&nbsp;&nbsp;Kinerja RM untuk akuisisi Giro s/d periode ]=]..posisi_report1..[=[</h2>
 <table cellspacing='0'>
	<thead>
		<tr>
			<th>PN - Nama RM</th> 
...
fo2:write([=[
 ...
 <br>
 &nbsp;&nbsp;&nbsp;<b>Rekening Giro tanpa PN Pengelola</b><br>
 <textarea>]=]
 ...
 <hr>
 <div align='center' style='font-size:smaller'>BRI Reporting Tool: DI321 EDW<br>Copyright &copy;2022, <b>Dhani Novan</b> (dhani_novan@bri.co.id)</div><br/>
</body>
</html>
]=])
fo2:close()
