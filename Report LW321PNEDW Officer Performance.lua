-- Display Performance each funding officer based on LW321PN EDW Current Account Balance (CSV Format from DWH BRISIM)
-- There is bug in report LW321PN. There are double data. So we have to remove duplicate date. We can use Notepad2 : Edit -> Block -> Sort Lines : Merge Duplicate Line
...
local OUTPUT_FILE = "Pinjaman_Bulanan.htm"
local OUTPUT_NO_PN_FILE = "Pinjaman_tanpa_PN.csv"
local output_sep = "|"

local res, tmp, ReportFileName1 = iup.GetParam("Pick Reports LW321 in CSV Format", nil, [=[
Sumber Data : %m\n
%t\n
Report LW321 : %f[OPEN|*LW321*.csv;*LW321*.txt;*LW321*.gz|CURRENT|NO|NO]\n
]=]
,"1. Buka Aplikasi BRISIM (https://brisim.bri.co.id)\n2. Pilih: DWH Reports\n3. Pilih: Tables\n4. Pilih No 4 LW321 - PN Laporan Kolektibilitas dan Tunggakan Per AO (1 Row)\n6. Download dan Save dalam format CSV", "E:\\BRI-Report\\data\\LW321PNSingleRow_V2.1 no_report 4.csv")
...
for line in f_lines1(ReportFileName1) do
	...
	if no == 1 then
		...
		if report1_type ~= "DWH-4-LW321" then
			iup.Message("Error","Report yang dipilih harus Report No 4 [LW321 - PN Laporan Kolektibilitas dan Tunggakan Per AO (1 Row)] dari DWH Reports dalam format CSV.\nSilahkan download ulang dari BRISIM atau \npilih kembali report yang sesuai.")
			return -1
		end
	else
		-- process data
		...
		if f[1] ~= "" then
			posisi_report1 = f[1]
			this_year = string.sub(posisi_report1,1,4)
			acc_dateopen = f[13]	-- dd/mm/yyyy
			if string.sub(acc_dateopen,7) == this_year then
				local lRM
				
				acc_RM = f[26].." - "..f[27]	-- PN RM SME
				...
				if list_RM[acc_RM] == nil then
					...
					lRM = list_RM[acc_RM]
					lRM[mm] = 1
					lRM[mm+12] = string.format("%s - %s [%s] Rp. %s",f[6] ,f[7], f[5], f[8])
					lRM["total_account"] = 1
					lRM["total_balance"] = acc_balance
				else
					lRM = list_RM[acc_RM]
					lRM[mm] = lRM[mm] + 1
					lRM[mm+12] = lRM[mm+12]..'&#13;'..string.format("%s - %s [%s] Rp. %s",f[6] ,f[7], f[5], f[8])
...
this_month = tonumber(string.sub(posisi_report1,6,7))
...
<title>BRI Reporting Tool: Kredit Baru dari data LW321</title>
<style type="text/css"> 
...
window.onload = function () {makeAllSortable();};
 ...
 <body>
 <h2>&nbsp;&nbsp;&nbsp;Kinerja RM untuk ekspansi kredit baru s/d periode ]=]..posisi_report1..[=[</h2>
 <table cellspacing='0'>
	<thead>
		<tr>
			<th>PN - Nama </th> 
...
fo2:write([=[
 ...
 <hr>
 <div align='center' style='font-size:smaller'>BRI Reporting Tool: LW321 EDW<br>Copyright &copy;2022, <b>Dhani Novan</b> (dhani_novan@bri.co.id)</div><br/>
</body>
</html>
]=])
fo2:close()
